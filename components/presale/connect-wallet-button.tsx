...
export function ConnectWalletButton({
  ...
  const handleOpenConnectModal = (openConnectModal: () => void) => {
    ...
    // No primeIosWalletWindow — blank tab was confusing users
    openConnectModal()
  }
        ...
        if (!connected) {
          return (
            <button
              ...
              className={`relative inline-flex items-center justify-center gap-2 rounded-xl font-bold uppercase overflow-hidden transition-all duration-300 ${
                variant === "header" ? "btn-outline-gold" : "btn-wallet"
              }`}
              style={{
